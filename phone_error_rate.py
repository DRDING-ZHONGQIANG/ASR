...
def add_noise(audio_dir, noise_dir, cfg_filedir, louder=0, softer=0):
  ...
  clean = librosa.load(audio_dir, sr=16000)
  clean = clean[0]
  noise = librosa.load(noise_dir, sr=16000)
  noise = noise[0]
...
def compare_phn_wrd_noise(timit_wav,
  ...
  output_lst = [["Initial", initial_per[0]['PER'], "PER"], ["Initial", initial_wer[0]["PER"], "WER"]]
...
def compare_phn_wrd_noise_multi(audio_dict,
  ...
  for DR_index in list(audio_dict.keys())[DR[0]:DR[1]]:
    for speaker in tqdm(list(audio_dict[DR_index].keys())[SPK[0]:SPK[1]]):
      for i in range(len(audio_dict[DR_index][speaker]['phn'])):
        f1 = audio_dict[DR_index][speaker]['phn'][i]
        f2 = f1.replace('.phn', '.wav')
        compare_df = compare_phn_wrd_noise(timit_wav=f2,
